...
// =(등호) : 대입 연산자
// A = B -> B를 A에 대입 (항상 오른쪽 값을 왼쪽 변수에 대입)
...
console.log("nuber1 + number2 =", number1 + number2);

// 선언된 변수는 콘솔에서도 사용 가능

// (에러 예시) -> number3 is not defined
...
/* 1. var */
/* 변수, 변수명 중복가능 O(덮어쓰기), 함수 레벨 범위(scope) */
function varCheck(){
  var menu = "콩국수";
...
/* 2. let */
/* 변수, 변수명 중복 허용 X, 블록{} 레벨 범위 */
function letCheck(){
  let name = "홍길동";

  console.log("name : ", name);

  /* let name = "김미영"; */ /* 주석 해제 시 오류 발생 */
  name = "김미영";
  // 블록 범위 변수 'name'을(를) 다시 선언할 수 없습니다.
  console.log("변경된 name : ", name); /* 그래도 입력하면 name 변수가 이미 선언됨 이라 나옴 */
  /* 해결방법 
        1. 변수에 대입된 값을 바꾸는게 목적 -> let 제거 ( name = 김미영) 으로 
        2. 새로운 변수 생성이 목적 -> 변수명 변경 (let name2 = 김미영) 으로 */
...
/* 3. const */
/* 상수(constant, 항상 같은 수), 변수명 중복 허용 X, 블록{} 레벨 범위 */
...
function constCheck(){
  ...
  /* 02_변수와자료형.js:84  Uncaught TypeError: Assignment to constant variable.
    at constCheck (02_변수와자료형.js:84:16) */
    // -> 상수 변수에 값이 할당됨
    //  --> 상수는 값을 다시 대입(할당)할 수 없어서 오류가 발생한다
    // 해결 방법 : 80 줄의 const nationalCode를 let nationalCode으로 변경

  console.log("국가 코드 : ", nationalCode);
}


/* var, let, const 의 scope(범위) 확인 */
...
function scopeTest(){
  ...
  if(num1 == 100){
    ...
    // var로 선언되었기 때문에 블록{}레벨이 아니라 함수{} 레벨 지역 변수
    // -> if 문이 끝나도 함수가 끝난게 아니기 때문에 함수 {} 안에서 어디서든 사용 가능
  }
  ...
  // num3 변수에 저장된 값이 300과 같아면 {} 내부 코드 수행
  if(num3 ==300){
    let num4 = 400; /* 블록{} 레벨 지역 변수 == 블록을 벗어나면 사용할 수 없다
                                              == 블록 내부에서만 사용 가능 */
    console.log("if 내부 num4 : ",num4);
  }

  console.log("num4 : ", num4)
    // 여기까지 후 범위테스트 버튼 클릭하면 ReferenceError: num4 is not defined
...
function scopeTest2(){
  ...
  if(1 == 1){ // 1과 1이 같다면 if {} 수행
    ...
    fn2 = "변경된 fn2"
    // [좁은 범위의 데이터를 넓ㅅ은 범위에서 사용할 수 있게 하는 방법]
    // if {} 내부(좁은 범위)에서 
    // 함수 {} 내부(넓은 범위)의 변수 값을 변경

    // 함수 지역 변수 temp와 같은 이름의 if 지역변수 temp를 선언
    const temp = 456;

    console.log("if 내부의 temp : ",temp); //456 나옴
...
function typeCheck(){
  ...
  /* undefined : 정의되지않은 변수 / 값 대입 X 또는 존재 X */
