...
function compoundCheck(){
  ...
  number -= 60; // number = number - 60 ; -> 60
  console.log(`number -= 20 : ${number}`); //60

  number *= 3; // number = number * 3 ; -> 180
  console.log(`number *= 3 : ${number}`); //180

  number /= 10; // number = number / 10 ; -> 18
  console.log(`number /= 10 : ${number}`); //18

  number %= 5; // number = number % 5 ; -> 3
  console.log(`number %= 5 : ${number}`); //3
}


/* 
  AND(&&) : 둘 다 true 인 경우에만 ture (나머지 상황은 다 false)
  OR(||) : 둘 다 false인 경우에만 false (나머지 상황은 다 true)
*/

/** AND 연산자 확인 */
function andCheck(){

  // ex) 104는 100 이상의 숫자이면서 짝수인가?
